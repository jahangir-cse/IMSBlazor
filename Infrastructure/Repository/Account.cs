...
{
    public class Account(UserManager<ApplicationUser> userManager, SignInManager<ApplicationUser> signInManager) : IAcccount
    {
        ...
        public async Task<ServiceResponse> LoginAsync(LoginUserRequestDTO model)
        {
            ...
            if (!verifyPassword.Succeeded) return new ServiceResponse(false, "Incorrect credentias provided");
        ...
        public async Task<IEnumerable<GetUserWithClaimRersponseDTO>> GetUsersWithClaimsAsync()
        {
            var UserList = new List<GetUserWithClaimRersponseDTO>();
            ...
            foreach (var user in allUsers)
            {
                ...
                if (getCurrentUserClaims.Any())
                    UserList.Add(new GetUserWithClaimRersponseDTO()
                    {
                        UserId = user.Id,
